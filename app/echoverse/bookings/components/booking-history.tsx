...
export const BookingHistory = () => {
  ...
  const [selected, setSelected] = useState<SharedSelection>(
    new Set(statusChoices.map((status) => status.value)),
  );
  ...
  const handleSort = (column: string) => {
    setFilter((prevState) => ({
      ...
      sort_by: column,
      sort_order:
        prevState.sort_by === column && prevState.sort_order === "asc"
          ? "desc"
          : "asc",
    }));
  };
    ...
    <div className="">
      <div className="mt-3 space-y-3 mb-2">
        <div className="flex justify-between flex-row-reverse items-end">
          ...
          <Input
            ...
            endContent={
              <Button isIconOnly variant="light" onPress={handleSearch}>
        ...
        <p className="text-xs text-white/50">
          Bookings Count: {bookingHistory?.results.length}
        </p>
      </div>
      <Table
        fullWidth
      ...
      >
        <TableHeader>
          ...
          <TableColumn onClick={() => handleSort("event_date")}>
            <div className="flex items-center gap-2">
              <p>Event Date</p>
              {filter.sort_by === "event_date" &&
          ...
          <TableColumn onClick={() => handleSort("status")}>
            <div className="flex items-center gap-2">
              <p>Status</p>
              {filter.sort_by === "status" && filter.sort_order === "asc" ? (
                <IoChevronDown size={15} />
              ) : (
                <IoChevronUp size={15} />
              )}
