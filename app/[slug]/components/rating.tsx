    ...
    return (
        <div className="flex items-center gap-3">
            ...
            {Array.from({ length: fullStars }, (_, index) => (
                <IoStar key={index} size={20} className="text-yellow-400" />
            ))}

            {hasHalfStar && <IoStarHalf size={20} className="text-yellow-400" />}

            {Array.from({ length: emptyStars }, (_, index) => (
                <IoStarOutline key={index} size={20} className="text-yellow-400" />
            ))}
        </div>
    );
};
