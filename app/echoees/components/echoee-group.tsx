...
export default function EchoeeGroup({ title, filters }: Props) {
  ...
  const { data: paginatedEchoees, isLoading, refetch } = useFetchArtistsWithFilterQuery({
    ...filters,
    page: page,
  });
  ...
  useEffect(() => {
    ...
    return () => scrollContainer?.removeEventListener("scroll", checkArrowVisibility);
  }, []);
  ...
  return (
    <>
      ...
      <div className="w-full relative group">
        <div ref={scrollRef} className="flex gap-2 overflow-x-scroll scrollbar-hide">
          {echoees.map((echoee) => (
            <EchoeeCard key={ echoee.id } echoee={echoee} />
          ))}
          {isLoading && page == 1 && <div className="flex gap-2">
...
{showRightArrow && !isLoading && echoees.length > 4 && paginatedEchoees?.has_next &&
          <button
            onClick={onArrowRightClick}
            className="absolute invisible group-hover:visible top-[50%] -translate-y-[50%]  right-2"
          >
            <BsChevronRight className="text-white/20 hover:text-white transition-all duration-300 rounded-lg hover:bg-white/5 p-2" size={80} />
          </button>
}
        ...
        {showLeftArrow && (
          ...
          >
            <BsChevronLeft className="text-white/20 hover:text-white transition-all duration-300 rounded-lg hover:bg-white/5 p-2" size={80} />
          </button>
        )}
      </div>
    </>
