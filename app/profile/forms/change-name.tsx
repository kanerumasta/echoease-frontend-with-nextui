...
export const ChangeName = ({user}:Props) => {
    const [updateName,{isSuccess, isError}] = useUpdateNameMutation()
    ...
    useEffect(()=>{
        ...
        if(isError){
            toast.error("Changing name failed. Please try again.")
        }
    },[isSuccess, isError])
                        ...
                        <ModalBody>
                            <form onSubmit={handleSubmit(onSubmit)}>
                                <div className="space-y-2">
                                    ...
                                    <Input {...register("first_name")} isInvalid={!!errors.first_name} errorMessage={errors.first_name?.message} radius="sm" size="lg" label="Enter new first name" placeholder={user.first_name}/>
                                    <Input {...register('last_name')} isInvalid={!!errors.last_name} errorMessage={errors.last_name?.message} radius="sm" size="lg" label="Enter new last name" placeholder={user.last_name}/>
                                </div>
                                <div className="flex items-center my-4 gap-2 justify-end">
                                    <Button radius="sm" type="button" startContent={<MdCancel />}>Cancel</Button>
                                    <Button radius="sm" color="primary" type="submit" startContent={<FaSave/>}> Save</Button>
                                </div>
                            </form>
                        </ModalBody>
                    </ModalContent>
